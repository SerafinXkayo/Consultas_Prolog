...
estado1(X):-ve_peliculas(X,suspenso),consume(X,palomitas),cuenta_netflix(X,si),tiempo_libre(X,si),tiene_tardeperfecta(X,si).

vida_saludable(X):-consume(X,ensalada),cuenta_netflix(X,si).

disfruta_camino(X):-escucha_musica(X,regional),tiene_audifonos(X,si),cuenta_spotify(X,si).

es_feliz(X):-cuenta_netflix(X,si),consume(X,maruchan),tiempo_libre(X,si).
